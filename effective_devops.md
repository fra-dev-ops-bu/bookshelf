...
### 3.1 オペレーターとしての開発者
1940年代、第2次世界大戦の勃発とともにコンピュータの実用が始まった。
...
1961年、アポロ計画が始動し10年以内に月を目指すことになり、ロケットに搭載するコンピュータシステムの開発が開始された。
- しかし、実務を担当するNASAにはオンボードフライトソフトウェアを書ける人材がいなかった。
- そこで、MITの数学者マーガレット・ハミルトンに協力を求めソフトウェア開発のリーダーとした。
- ただし、ハミルトン自身もそのような複雑なソフトウェアを開発したことはなかった。
- 彼女の言葉 `開拓者になる以外選択肢がなかったんですから。初心者でいられる時間はなかったのです。`
- この開発の過程で**ソフトウェア工学**や**プライオリティディスプレイ**(リアルタイムに宇宙飛行士に警告を知らせるソフトの仕様) の概念を生み出した。
- 彼女は要求を取りまとめ、課題リストに次の3つの品質保証に関するタスクを追加した。
...
# 第VI部 devops 文化への架け橋
1969年、アポロ11号月面着陸船の着陸誘導ソフトウェアは当時のハードウェアでは扱いきれない計算をしなければならなかった。
- そこで、開発チームはソフトウェアの動作を手動で上書きできるようにプログラムした。
- つまり、ニール・アームストロング船長が手動制御で月面着陸船を操縦できるようなシステムにした。この機能がなければこの歴史的偉業の達成は不可能だったとのちに判明している。
- このソフトウェア開発が成功したのは、なによりも上層部がこのソフトウェア開発チームに与えた**自由と信頼**、チームメンバー同士の互いに相手を敬う気持ちがあったからだ。

### 3.3 プロプライエタリソフトウェアと標準化の登場
1964年、IBMがSystem/360という汎用性のあるコンピュータを発表。
- ここでの目標は、製品開発、製造、サービス、サポートにかかるコストを削減すること、必要に応じて顧客がアップグレードできるようにすることだった。
- 見事にそれらを達成し、市場を支配するmain frame computerとなった。
- System/360の登場により、それまで密接に関係していたソフトウェアとハードウェアを別々に学べるようになり、新たな触手 (ソフトウェのみのエンジニア) が誕生した。

1960年代末まで、コンピューターはリース契約が普通だった。
- 当時、製品としてはソフトウェアよりもハードウェアの方が重視されていたためソフトの料金は明示されておらず (ハードの料金に含まれる)、ソースコードも広く提供されていた。
- しかし、1969年にIBMが反トラスト法 (いわゆる独禁法。自由競争を阻害する独占や取引制限などを禁止するアメリカの法律) 違反で訴追された。ソースコードの公開は自由競争を阻害すると考えられたのだ。
- これを契機として、IBMはソフトウェアにも料金を課すようになった。
- この影響は大きく、ソフトウェア自体が大きな金銭価値を持つものとなり、オープンに提供されなくなった。

### 3.4 ネットワークの時代
1979年、Usenetという世界規模の分散ディスカッションプラットフォームがデューク大学の学生２人により始められた。
- 
