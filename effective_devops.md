...
#### 13.1.3 間違ったツールを選ばない様に注意しなければならない
ツールの細部に重点を置くのではなく、そのツールの使い方のPros/Consについて注意を払う必要がある。

ツールの適切な使い方や、組織へツーるを定着させる方法についての原則を理解し、新しいツールや技術の導入に柔軟になることで、最適でないツールを選んだ場合でも対処へリソースを割きすぎる必要がない。
#### 13.1.4 devopsツール全部入りセットやdevops-as-a-serviceを買ってくればよい
devopsは単なるツールの話ではない。また、devopsは文化活動であり、サービスとして購入するものではない。

devopsを売りにするツールを導入するときは、4本柱を基に、そのツールがチームに対してどのように機能するかを理解していく必要がある。

### 13.2 ツールの問題解決
欲求的な理由ではなく、問題解決のために役立つという理由でツールを選ぶことが重要だ。

#### 13.2.1 技術Xのベストプラクティスを見つけようと努力している
問題解決に飛び込む前に以下の事を行い、問題解決の選択肢を比較し選択していくことが重要である。
- 問題の現在の状態をつかむ
- 必須のものと、あれば便利なものを明確に分ける
- 重要な情報を持っている個人やチームを明らかにして、彼らの力を借りて問題を評価する

問題解決の意思決定のプロセスをドキュメント化することで透明性と意識的な決定プロセスを明確にすることが出来る。ドキュメントには注目したこと、問題点、ifプランなども含めて考えると、予想外の問題にも頑健なソリューションを導き出せる。

#### 13.2.2 ひとつのツールにする合意が得られない
チームの規模にかかわらず、全員一致の合意は簡単には得られない。また、エンジニアは使いやすいツールからの変更に反発する場合もある。ツールの統一や選択には以下を検討することが有効である。
...
>水研の場合は情報漏洩の問題からフリーのサービスや海外サーバーのサービスなどの導入に慎重だ。仕事の内容から考えると仕方ないが、柔軟に採用できるところは採用していきたい。

#### 13.2.3 技術Xの採用（または廃止）を決めたが、社員がそれに抵抗している
新ツールの使用に賛成されるかどうかは、ツール選択のプロセスに大きく左右される。持続可能で有益な変更を生み出すためには、コミュニケーションと共感が大きな意味を持つ。そのためには、以下の様なことが重要となる。
- ツール使用者のフィードバックの取得
