...
そこで、devops文化が浸透したある米国企業のやり方をみてみよう:
...
Webサービスとは、Amazonを例にとれば、あのサイトで我々ができること全てだ。
IT系の大企業がみなWebサービスを持っているのは、現時点で価値を効果的に届けるための最高の方法だからだ。
...
ユーザーの体験には影響を与える心配をせずに仕事に集中でき、変更の結果を手元で確認できる。
仕事が終わると、開発者は成果を「テスト環境に」送る。
他の開発者も並行して仕事をしていることがほとんどなので、全ての変更がうまく噛み合うかを試験するわけだ。
テスト環境でも動作が確認できたら、開発者の仕事は本番環境に反映される。
