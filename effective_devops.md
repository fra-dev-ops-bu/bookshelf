...
### 3.1 オペレーターとしての開発。
- 1940年代第二次世界大戦の勃発とともにコンピュータの実用が始まった。
- 最初のコンピュータはENIAC (Electronic Numeric Integrator and Computer) と呼ばれ18000本の真空管からなる超大型ハードウェアだった。
- 操作方法は40枚のコントロールパネルのダイヤルを設定し配線を繋ぎ変えるというものでキーボードやマウスなどは当然無かった. 弾道計算に使われた。

### 3.2 ソフトウェアエンジニアリングの始ま。
...
### 3.3 プロプライエタリソフトウェアと標準化の登。
### 3.4 ネットワークの時。
### 3.5 グローバルなコミュニティの始ま。
### 3.6 アプリケーションとウェブの時。
### 3.7 ソフトウェア開発手法の発。
### 3.8 オープンソースソフトウェアとプロプライエタリサービ。
### 3.9 アジャイルインフラストラクチャ。
### 3.10 DevOpsDaysの始ま。
### 3.11 devopsの現。
### 3.12 まと。
