...
システム思考はソフトウェア開発や運用のみならず全体を扱うものである。代表的な手法である **リーン生産** は価値を与える部分に注目すると共に、以下のムダを取り除くことに全力を注ぐ手法である。
...
リーンとは「痩せた」を意味する言葉、すなわち無駄を取り除くということにつながる。水研の仕事はどうだろうか？不要な引継ぎ、学習のやり直し、得手不得手を超えた超人技術の要望、10年以上前のレガシーエクセル等、無駄が多いのではないだろうか？frasyrのVignetteによって計算される速度と、ExcelのSolverを使って結果と図表を出力する速度を見たとき、どちらがより「リーン」なのかは明白だ。

### 4.4 開発、リリース、デプロイの諸概念

devops文化は様々な要素によって支えられている。以下に代表的なものをまとめる。

| 概念 | 説明 |
|-----|-----|
| バージョン管理(VCS) | ファイルの変更、日時、誰が行ったかなどのの履歴を記録し保存するシステム。過去のCommit(変更)の復元が可能なため、過去のCommit(変更)の復元が可能能とする。 |
| テスト駆動開発(TDD) | 開発に際し、最初にテストを書くことで、新しい機能を明確に定義し、コードを洗練させる開発手法。まずテストを通るコードを書き、洗練・抽象化させていくことによりバグやエラーの少ないコード開発が可能となる。
| アプリケーションのデプロイ | ソフトウェアリリースの計画、実行、保守のプロセス。変更や新機能のデプロイは依存する機能へ影響を及ぼす。依存関係の構築を自働化することでリスクを最小化することが出来る。 |
| 継続的インテグレーション(CI) | 開発されたコードを頻繁にマスターに統合するプロセス。変更点を細かに記載しマージすることで大きなエラーやコンフリクトを予防する。また、コミットやマージにあたり自動テストを行うことで問題の特定と修正を可能する。 |
| 継続的デリバリー(CD) | 自動テストとCIを統合し、リリースプロセスを自働化する手法。新たな変更に対してテストを通過させることでデプロイが可能であることを保証する。|
| 継続的デプロイ(CD) | 自動テストなどを用いて、変更を本番環境にデプロイするプロセス。変更の検証とテスト合格、本番換気用へのデプロイの一連の動作を自動で行う。 |
| MVP(実用最小限の製品) | 製品のアイデアを確かめるために、必要最小限の労力で製品のプロトタイプを作るという考え方。顧客が望まない製品の開発を回避し、最小限の投資で顧客に関する情報を収集することでコストとムダを削減する。|

### 4.5 インフラストラクチャーに関する概念

仕事の多くは何かしらのインフラストラクチャーの上で実行される。自前のコンピューターやクラウド環境がそれにあたる。開発と運用が連続的になるdevopsでは、インフラストラクチャーの理解は重要となる。

- **構成管理** は製品のライフサイクルの最初から最後まで、識別、管理、監視、監査するプロセスである。
  - 一貫した環境や要素を構築することでパフォーマンスを確立させる。
- **クラウドコンピューティング** はインターネットを経由してコンピューター機能や処理能力をサービスとして受け取ることができるインフラストラクチャーである。
  - 柔軟かつ利便性がある他、統一した環境での作業を行うことが可能である。
  - devopsの文化と親和性が高いが、自分にとって効果的なものを用いて開発するdevopsにとってクラウドコンピューティングは必須ではない。
- **インフラストラクチャー自動化** はシステムの保守や管理を自動化させることで失敗と労力を削減する手法である。
  - 反復的な手作業の混乱とエラーを取り除き効率的な運用が可能。
- **アーティファクト管理** はソフトウェア開発プロセスでの生成されたバイナリやパッケージをレポジトリ等で管理する手法である。
- **コンテナ** は仮想環境で開発やデプロイを行うことができる技術である。
...
実質個人事業主の様な水研では、レトロスペクティブの様なミーティングは珍しい。そもそも超人的な能力で「物事をミスなくこなす」ことができる研究者の多い組織であるため、ミスを事前に修正することができるからだ。しかし全員が超人ではないため、ミスを多発しやすい仕事環境に置かれた多くの資源評価担当者は資源評価の時期になると焦ってしまう。レトロスペクティブを行うことで、プロジェクトチーム全体の学習を推進することができるほか、失敗を応用することが可能となるため、もっと情報の共有が必要だろう。
...
devopsとして重要な点は、ポストモーテムを非難の場にしてはならない。インシデントの責任を押し付けることは、学習として効果的とは言えないからだ。
...
組織的な学習は非難のない文化をつくり、ミスやバグから経験や教訓を導き出すことが出来る。組織として、また組織的な学習は大きな影響をもたらし、成功することが出来る。

##### FDOB活動と組織的な学習

fra-dev-ops-buは組織的な学習といえるのだろうか。1人で学習するより3人で学習することで相乗効果が感じられ、互いに前進する切っ掛けを与えてくれる。組織的な学習は個人的な学習を促進し、学習はより自らの視点をより高い視点へ移すことが可能になる。水研として生態学や水産資源学の勉強会は行われているが、より高い視点の勉強会も今後は頻繁に行われるべきである。
